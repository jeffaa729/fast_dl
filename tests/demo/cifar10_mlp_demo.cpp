...
class Cifar10MLP : public dl::nn::Module {
public:
    Cifar10MLP(const dl::Device& device, dl::nn::LinearInit init)
        : fc1_(3 * 32 * 32, 256, device, init),
          relu_(),
          fc2_(256, 10, device, init) {
    ...
    dl::Tensor forward(const dl::Tensor& input) override {
        dl::Tensor flattened = dl::ops::flatten(input);
        return fc2_(relu_(fc1_(flattened)));
    }
...
Metrics evaluate(Cifar10MLP& model,
...
int train_one_epoch(Cifar10MLP& model,
                    demo::cifar10::Cifar10DataLoader& loader,
...
void print_sample_results(Cifar10MLP& model,
                          demo::cifar10::Cifar10DataLoader& loader,
                          int sample_count) {
    dl::autograd::NoGradGuard no_grad;
    model.eval();
    ...
    for (int i = 0; i < count; ++i) {
        std::cout << " [" << i << "] pred="
                  << predictions[static_cast<std::size_t>(i)]
...
int main() {
    ...
    try {
        const int epochs = env_int("CIFAR10_EPOCHS", 5);
        const int batch_size = env_int("CIFAR10_BATCH_SIZE", 64);
        const int max_train_batches =
            env_non_negative_int("CIFAR10_MAX_TRAIN_BATCHES", 0);
        const int max_test_batches =
            env_non_negative_int("CIFAR10_MAX_TEST_BATCHES", 0);
        const int train_eval_batches =
            env_non_negative_int("CIFAR10_TRAIN_EVAL_BATCHES", 20);
        ...
        Cifar10MLP model(device, linear_init);
        dl::optim::SGD optimizer(model.parameters(), learning_rate);

        std::cout << "cifar10_mlp_demo : start\n";
        ...
        std::cout << "cifar10_mlp_demo_error : " << error.what() << "\n";
        passed = false;
    } catch (...) {
        passed = false;
    }

    std::cout << "cifar10_mlp_demo : "
