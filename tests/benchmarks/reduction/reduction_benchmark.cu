...
bool run_and_validate_reduction(dl::bench::DeviceBuffer<float>& device_input,
                                dl::bench::DeviceBuffer<float>& device_output,
                                float* gpu_result, const float* cpu_result,
                                std::size_t output_size, std::size_t size) {
    dl::kernels::reduction(device_input.data(), device_output.data(), size);
...
int reduction_benchmark(std::size_t size) {
    ...
    const bool valid = run_and_validate_reduction(
