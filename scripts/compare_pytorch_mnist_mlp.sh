...
MNIST_TRAIN_EVAL_BATCHES="${MNIST_TRAIN_EVAL_BATCHES:-20}"
...
end_ns="$(date +%s%N)"
dl_total_ms="$("${PYTHON_CMD}" - "${start_ns}" "${end_ns}" <<'PY'
import sys
...
dl_avg_epoch_ms="$("${PYTHON_CMD}" - "${dl_total_ms}" "${MNIST_EPOCHS}" <<'PY'
import sys
...
dl_train_batches="$("${PYTHON_CMD}" - "${MNIST_MAX_TRAIN_BATCHES}" "${MNIST_BATCH_SIZE}" <<'PY'
import sys
...
"${PYTHON_CMD}" tests/benchmarks/pytorch/pytorch_mnist_mlp_benchmark.py

echo
echo "compare : mnist mlp training"
"${PYTHON_CMD}" tests/benchmarks/compare/compare_mnist_mlp_training.py
